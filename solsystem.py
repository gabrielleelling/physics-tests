...
BODY_MASS = 100
ORBIT_RADIUS_INCREMENT = 1  # 1 inch (or any desired unit) increment in orbit radius for smaller bodies
...
for i in range(NUM_BODIES):
    ...
    orbit_radius = CENTER_BODY_RADIUS + (i + 1) * ORBIT_RADIUS_INCREMENT  # Increment orbit radius for smaller bodies
    x = center_body.x + orbit_radius * math.cos(angle)
    y = center_body.y + orbit_radius * math.sin(angle)
    radius = BODY_RADIUS
    mass = BODY_MASS
    color = BODY_COLOR
